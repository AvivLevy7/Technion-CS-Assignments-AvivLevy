...
static void _init_blocks() __attribute__((constructor));
...
void* smalloc(size_t size) {
    ...
    if (size > MAX_BLOCK_SIZE - sizeof(MetadataBlock)) {
        Metadata metadata = (Metadata)mmap(nullptr, size + sizeof(MetadataBlock), PROT_READ | PROT_WRITE | PROT_EXEC, MAP_ANONYMOUS, -1, 0);
